...
with col2:
    ...
    too_liik = st.selectbox("Tehtav töö põhiliik", [
        "Hoolduslõikus (kõrghaljastus)", 
        "Sektsiooniline langetamine", 
        "Suunatud langetamine", 
        "Ilu- ja pargipuude istutamine", 
        "Haljastus- ja pinnasetööd", 
        "Hekkide pügamine", 
        "Viljapuude hoolduslõikus", 
        "Parkide ja haljasalade hooldus", 
        "Kändude freesimine", 
        "Võsa lõikus"
    ])
...
with col5:
    tehnika = st.selectbox("Põhitehnika", ["Käsitööriistad (labidad, käärid)", "Mootorsaag / Hekilõikur", "Miniekskavaator / Laadur", "Mootortigu (aukude puurimiseks)", "Kännufrees / Hakkur", "Tõstuk / Kraana"])
...
with col6:
    ergonoomika = st.selectbox("Terviseriskid", ["Ei ole", "Raskete koormate tõstmine (istutusmuld/pallid)", "Pidev sundasend", "Masinate vibratsioon", "Lendav praht / Tolm"])
...
def create_pdf():
    ...
    # Tabeli stiilis andmed
    def draw_row(pdf, label, value):
        pdf.set_font("Helvetica", 'B', 10)
        pdf.cell(50, 8, label, 1)
        pdf.set_font("Helvetica", '', 10)
        pdf.cell(140, 8, str(value), 1, 1)

    draw_row(pdf, "Ettevõte", ettevote)
    draw_row(pdf, "Objekt", aadress)
    draw_row(pdf, "Töö liik", too_liik)
    draw_row(pdf, "Vastutav", vastutav)
    draw_row(pdf, "Kuupäev", kuupaev)
    pdf.ln(5)

    pdf.set_fill_color(230, 240, 230)
    pdf.set_font("Helvetica", 'B', 11)
    pdf.cell(0, 10, " RISKIANALÜÜSI TULEMUSED", 1, 1, 'L', True)
    ...
    for r_label, r_value in riskid:
        pdf.cell(60, 8, r_label, 1)
        pdf.cell(130, 8, r_value, 1, 1)
